...
| Seção | O que faz |
|---|---|
| **PageSpeed** | Chama a API pública do Google (mobile e desktop): scores de Performance, Acessibilidade, Boas Práticas, SEO + Core Web Vitals + top oportunidades de melhoria |
| **Tags** | Detecta contêineres GTM, propriedades GA4, IDs de Google Ads e Meta Pixel no HTML; sinaliza duplicidade de tags |
...
- A ferramenta lê apenas o HTML estático da página inicial informada. Sites
  que renderizam formulários/tags via JavaScript pesado (SPAs em React/Vue sem
  SSR) podem não ter tudo detectado.
